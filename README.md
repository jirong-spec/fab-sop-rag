...
> **限制**：3 SOP / 48 邊的圖太小，2-hop 遍歷幾乎撈回整張圖，retrieval 沒被真正壓測（圖規模所限）。LLM-judge 與生成用同一個 vLLM（self-grading bias），keyword 與 recall 為 model-independent 的交叉驗證。
...
### Token 預算裁切 + 邊 gloss enrichment

兩個與圖規模相關的設計決策：

1. **context 溢出防護**：圖變大、回傳 triple 變多時，prompt 可能超過 4096 context 預算。
...
2. **低相似度邊被砍**：`REQUIRES_STATUS / PRECONDITION / INTERLOCK` 邊只有 CamelCase ID，對中文問題相似度低 → 被 rerank cap 砍掉。
   → **邊 gloss enrichment**（`graph_store`）：為缺 `description` 的邊型合成中文 gloss（比照 NEXT_STEP/DEPENDS_ON），讓這類邊在 rerank 時不被壓到底。
