...
### 逐題比較（Live 實測，2026-05-13，Qwen2.5-7B-Instruct-AWQ-int4，bi-encoder reranking + 動態 cap + implicit CoT）

```
ID   │ 類別                        │ Graph RAG (R=檢索 A=回答)  │ Baseline RAG               │  Graph  │  Base
─────┼─────────────────────────────┼────────────────────────────┼────────────────────────────┼─────────┼────────
q01  │ anomaly_handling            │ R✅ A✅ 2/2               │ R✅ A✅ 2/2               │  4881ms │  1593ms
q02  │ sop_step_sequence  [↑HOP]   │ R✅ A✅ 4/4               │ R❌ A❌ 0/4               │  4349ms │  1220ms
q03  │ equipment_precondition      │ R✅ A✅ 4/4               │ R✅ A✅ 4/4               │  5012ms │  2236ms
q04  │ step_dependency             │ R✅ A✅ 3/3               │ R✅ A✅ 3/3               │  3102ms │  1449ms
q05  │ cross_doc_dependency [↑HOP] │ R✅ A✅ 2/2               │ R✅ A✅ 2/2               │  5404ms │  1026ms
q06  │ interlock_condition         │ R✅ A✅ 3/3               │ R✅ A⚠  1/3               │  6104ms │  2099ms
q07  │ vent_procedure      [↑HOP]  │ R✅ A✅ 2/2               │ R❌ A❌ 0/2               │  4855ms │  1260ms
q08  │ off_topic_blocked           │ ✅ blocked                │ ✅ blocked                │   585ms │   584ms
q09  │ off_topic_blocked           │ ✅ blocked                │ ✅ blocked                │   664ms │   663ms
q10  │ pump_check_sequence         │ R✅ A✅ 4/4               │ R❌ A⚠  1/4               │  5716ms │  1354ms
─────┼─────────────────────────────┼────────────────────────────┼────────────────────────────┼─────────┼────────
     │ TOTALS                      │ R:24/24(100%) A:24/24(100%)│ R:14/24(58%) A:13/24(54%)  │ avg 4067ms │ avg 1348ms
```
...
| **Answer 命中率**（有回答對） | **100%** (24/24) | 54.2% (13/24) | **+45.8 pp** |
| **多跳查詢 Answer 命中率** | **100%** (8/8) | 25.0% (2/8) | **+75.0 pp** |
| **正確攔截非 SOP 問題** | 2/2 | 2/2 | — |
| **平均端對端延遲** | 4067 ms | 1348 ms | +2719 ms |

> 以上數字為 Live 實測（Neo4j + vLLM **Qwen2.5-7B-Instruct-AWQ-int4** + Chroma 全服務啟動，2026-05-13），Graph RAG 採用 bi-encoder cosine reranking + 動態 cap（top_score × 0.5 閾值）+ implicit CoT prompt。Answer 命中率以 **answer 欄位**計算，R 命中率以 **model_triples**（模型實際收到的 triples）計算。評測關鍵字為語意實體（移除 schema 術語如 FIRST_STEP、PRECONDITION 等）。

**結論：**

- Graph RAG Retrieval 與 Answer 均達到 **100%**：bi-encoder reranking + 動態 cap 確保所有關鍵 triples 進入模型 context；implicit CoT 指令讓模型完整提取 triple 屬性（如 `interlock_id`、`trigger`、`action`、`reason`）。
- Baseline 只有 R 58% / A 54%，步驟鏈（q02、q10）和依賴鏈（q07）完全抓不到。
...
| Qwen2.5-7B-Instruct-AWQ-int4 + implicit CoT | **24/24 (100%)** | 4067 ms | +33% |
...
- **加入 implicit CoT 後達到 100%**（+8.3 pp）：一行 prompt 指令讓模型在回答前內部核對所有 triple 屬性，僅多 106 ms。

> 評測採用 bi-encoder reranking + **固定 cap=50**，準確率以 **model_triples**（模型實際收到的 triples）的 answer 欄位計算（2026-05-12）。現行 pipeline 改用動態 cap + implicit CoT，7B 準確率達 **100%**，延遲 4067 ms，詳見第四節。
...
| **Bi-encoder + Edge enrichment + implicit CoT（7B）** | **動態** | **100%** | **100%** | **4067 ms** ✅ |

\* 原始 R 是對全部 triples 計算（含模型未看到的），實際模型可見 R 約 71%。以上為 3B 結果（最後一行為 7B）。
...
- **Implicit CoT（一行 prompt 指令）** 將 Answer 從 91.7% 提升到 **100%**（+8.3 pp），僅多 106ms。顯式 scratchpad（方法 B）反而因 token 預算壓縮導致 q06 仍失敗；implicit CoT 讓模型在內部推理，512 token 全用於輸出
