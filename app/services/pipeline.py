...
def run_pipeline(req: AskRequest) -> AskResponse:
    ...
    logger.info("Retrieval done | entities=%s | triples=%d", entities, len(triples))
    ...
    if answer == LLM_ERROR_ANSWER:
        return _blocked(req, guardrail_results, "llm_error", answer, t0, entities=entities, triples=triples)
...
def _blocked(
    ...
    latency_ms = int((time.perf_counter() - t0) * 1000)
    logger.info("Pipeline blocked | reasoning=%s | latency_ms=%d", reasoning_type, latency_ms)
